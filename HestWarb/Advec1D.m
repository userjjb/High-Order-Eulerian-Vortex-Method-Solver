function [u, saved] = Advec1D(u, FinalTime, LogTime, TimeTol, alpha)

% Integrate 1D advection until FinalTime starting with
% initial the condition, u

Globals1D;
time = 0;

% Runge-Kutta residual storage  
resu = zeros(Np,K); 

% compute time step size
xmin = min(abs(x(1,:)-x(2,:)));
CFL=0.75; dt   = CFL/(2*pi)*xmin; dt = .5*dt;
Nsteps = ceil(FinalTime/dt); dt = FinalTime/Nsteps; 

% advection speed
a = 2*pi;

% outer time step loop
disp('Starting time loop')
iter=0;
saved=zeros(N+1,K,FinalTime/LogTime);
for tstep=1:Nsteps
    for INTRK = 1:5
        timelocal = time + rk4c(INTRK)*dt;
        %Pass all the params instead of using global vars, much faster than
        %redeclaring globals each run
        [rhsu] = AdvecRHS1D(u, timelocal, a, Nfp, Nfaces, K, vmapM, vmapP, nx, vmapI, mapI, mapO, rx, Dr, LIFT, Fscale, alpha);
        resu = rk4a(INTRK)*resu + dt*rhsu;
        u = u+rk4b(INTRK)*resu;
    end;
    % Increment time
    time = time+dt;
    %Crude logging of solution
    if mod(time,LogTime)<TimeTol
        iter = iter+1;
        saved(:,:,iter)=u;
    end
end;
disp('Time loop finished')
return
