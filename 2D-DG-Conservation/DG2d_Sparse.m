...
Gdy1=   0;
GA1=    1;
ICfuns{end+1}=@(x,y,t) GA1*exp(-(((x)-Gdx1).^2/Ga1+(y-Gdy1).^2/Gb1));%Center is at (dx,dy)
...
for t=0:delt:3
    ...
    if mod(t,skip)<delt
        ...
        text(-1,1,1.2,num2str(sqrt(sum(sum(norm_h.*R.^2)))));
        pause(0.001)
    end
end