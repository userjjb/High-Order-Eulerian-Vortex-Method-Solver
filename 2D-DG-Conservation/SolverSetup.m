...
kernel_x= permute(bsxfun(@minus,srcy,rv_xS(1,2,:,:,:))./(sum(bsxfun(@minus,rv_xS,[srcx,srcy]).^2,2)+2*del^2).^(3/2),[1 3 4 5 2]);
kernel_y= permute(bsxfun(@minus,rv_yS(1,1,:,:,:),srcx)./(sum(bsxfun(@minus,rv_yS,[srcx,srcy]).^2,2)+2*del^2).^(3/2),[1 3 4 5 2]);
