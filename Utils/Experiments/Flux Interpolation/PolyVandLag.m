...
function Lfun = PolyVandLag(N,n)
    ...
    Coeff_n =a./prod(Qx(n)-NOn); %Divide by the denominator of the Lagrange basis
    %Calculate Vandermonde matrix and multiply by our poly coeffs to
    %evaluate our Lagrange basis at each point 'x'
    Lfun= @(x) Coeff_n*(repmat(x',1,N).^repmat(0:N-1,length(x),1))';
end